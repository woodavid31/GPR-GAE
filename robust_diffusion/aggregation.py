...
def soft_median(
    ...
    **kwargs
    ...
    with torch.no_grad():
        # Calculate median index for each node based on neighbors
        unique_rows = torch.unique(row_index)
        median_idx = []
        for row in unique_rows:
            neighbors = col_index[row_index == row]  # Get neighbors for this row
            neighbor_features = x[neighbors]  # Features of the neighbors
            distances = torch.norm(
                neighbor_features.unsqueeze(1) - neighbor_features.unsqueeze(0),
                dim=-1,
                p=p
            )
            # Find the neighbor that minimizes total distance
            row_median_idx = neighbors[distances.sum(dim=1).argmin()]
            median_idx.append(row_median_idx)

        median_idx = torch.tensor(median_idx, device=x.device)  # Shape: [batch_size]
        median_idx = median_idx.unsqueeze(-1).expand(-1, d)  # Broadcast to feature dimensions

    # Extract features of median neighbors
    x_median = x[median_idx, torch.arange(d, device=x.device)]

    # Calculate distances for softmax weighting
    distances = torch.norm(
        x_median[row_index] - x[col_index], dim=1, p=p
    ) / pow(d, 1 / p)

    # Compute softmax weights
    soft_weights = torch_scatter.scatter_softmax(
        -distances / temperature, row_index, dim=-1
    )
    weighted_values = soft_weights * edge_weights
    row_sum_weighted_values = torch_scatter.scatter_add(weighted_values, row_index)

    # Normalize final adjacency weights
    final_adj_weights = (
