...
from robust_diffusion.models import GPRGNN
...
def train_inductive(model, attr_training, attr_validation, adj_training, adj_validation, labels_training, labels_validation, idx_train, idx_val,
    ...
    trace_acc_train = []
    trace_acc_val = []
    if isinstance(model, (GPRGNN)) and isinstance(model.prop1, GPR_prop) and model.prop1.norm == True: # exclude prop coeffs from weight decay
    ...
    model.train()
    for it in tqdm(range(max_epochs), desc='Training...',leave=False, disable=True):
        #### Train step ####
        optimizer.zero_grad()
        model.train()
