...
class LRBCD(SparseAttack):
    ...
    def _attack(self, n_perturbations,dgcn = False, **kwargs):
        ...
        for epoch in tqdm(range(self.epochs),leave=False, disable=True):
            ...
            if dgcn:
                ...
                _ = self._get_logits(self.attr, self.edge_index, self.edge_weight)
                logits_clean = self.attacked_model.original_logit()
                loss = 1*self.calculate_loss(logits[self.idx_attack], self.labels[self.idx_attack]) + 0*(self.kl(F.log_softmax(logits, dim=1),F.log_softmax(logits_clean, dim=1)))
            else:
